...
class ProductsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $categories = factory(Category::class, 4)->create(); // Se crean en BD  categorías
        $categories->each(function($c){ // Para cada una de las categorías se ejecuta esta función, donde a cada categoría se le asignan 5 productos 
            $products = factory(Product::class, 5)->make(); // Se crean 5 productos x categoria sin guardarlos en BD
            ...
            $products->each(function($p){ // Para cada uno de los productos de esta categoria se ejecuta esta funcion, donde a cada producto p se le asigna una colección de 5 imágenes
                $images = factory(ProductImage::class, 3)->make(); // Se crean 3 imágenes para cada producto
